...
This gives you the `kitz` CLI — your single entry point for creating
...
```bash
kitz dev       # Auto-reload on file changes (uses cargo-watch)
kitz run       # Build and run
kitz run --release  # Build and run in release mode
kitz test      # Run tests
kitz test --watch   # Re-run tests on changes
```

## Themes

```bash
kitz theme list     # Show all 4 themes with color swatches
...
kitz gives you all of that out of the box:
...
kitz grows with your app. Start minimal, add conventions as needed:
...
## Project Structure (generated by `kitz new`)
...
Marker comments (`// kitz:messages`, `// kitz:update`, etc.) serve
as injection points for the `generate` command. Don't remove them.

## Library Usage (without CLI)

If you prefer to use kitz as a pure library without the CLI binary:
